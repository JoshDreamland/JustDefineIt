...
int jdip::context_parser::handle_class(lexer *lex, definition_scope *scope, token_t& token)
{
  ...
  if (token.type == TT_CLASS)
    ...
    token.report_error(this, "PARSE ERROR: handle_class invoked with non-class, non-struct token.");
  #endif
  ...
  if (dins.second) // If a new definition key was created, then allocate a new class representation for it.
    ...
    if (not(dins.first->second->flags & DEF_CLASS)) {
      token.report_error(this,"Attempting to redeclare `" + classname + "' as a class");
      return 1;
    }
  ...
  if (token.type == TT_COLON) {
    if (already_complete) {
      token.report_error(this, "Attempting to add ancestors to complete class `" + classname + "'");
      return 1;
    }
    incomplete = 0;
    do {
  ...
  {
    incomplete = 0;
    if (handle_scope(lex, nclass, token, protection)) return 1;
    if (token.type != TT_RIGHTBRACE) {
      token.report_error(this,"Expected closing brace to class `" + classname + "'");
      return 1;
    }
  }
  else if (!incomplete) {
    token.report_error(this,"Expected class body here after parents named");
    return 1;
  }
