...
definition_union* context_parser::handle_union(
  ...
  string classname;
  definition_union *nunion = NULL; // The class definition. Will be NULL until allocation, unless the class was already declared.
  ...
  if (token.type == TT_LEFTBRACE)
  {
    ...
    if (handle_scope(nunion, token, 0)) FATAL_RETURN(NULL);
