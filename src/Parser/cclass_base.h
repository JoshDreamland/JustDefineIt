...
static inline int get_location(definition_name* &nclass, bool &will_redeclare, bool &already_complete, token_t &token, string &classname, definition_scope *scope, context_parser * const cp, error_handler *const herr) {
  ...
  else if (token.type == TT_DEFINITION || token.type == TT_DECLARATOR) {
    classname = token.def->name;
    token = cp->read_next_token(scope);
  }
  else {
    char buf[32];
  ...
  for (definition_scope *s = scope; s; s = s->parent) {
    ...
    if (it != s->c_structs.end()) {
      if (it->second->flags & DEF_FLAG) {
        nclass = (definition_name*)it->second;
        will_redeclare = (s != scope);
        already_complete = !(nclass->flags & DEF_INCOMPLETE);
      }
      else if (s == scope) {
        token.report_error(herr, "Using `" strname "' token to name " + it->second->kind() + " `" + classname + "'");
        return 1;
      }
      break;
    }
