...
namespace jdi {
  ...
  typeflag::typeflag(): usage(), flagbit(0), def(nullptr) {}
  typeflag::typeflag(string n, USAGE_FLAG u): name(n), usage(u), flagbit(0), def(nullptr) {}
  typeflag::~typeflag() { }
...
namespace jdi {
  ...
  {
    unsigned long flag;
    pair<tf_iter, bool> insit = builtin_declarators.insert(pair<string,typeflag*>(type_name,nullptr));
    if (insit.second) {
      insit.first->second = new typeflag(type_name, usage_flags);
      if (usage_flags & (UF_PRIMITIVE | UF_STANDALONE)) {
        if (prim_name.empty())
          prim_name = type_name;
        pair<prim_iter, bool> ntit = builtin_primitives.insert(pair<string,definition*>(prim_name,nullptr));
        if (ntit.second)
          ntit.first->second = new definition_atomic(prim_name, nullptr, DEF_TYPENAME, sz);
      ...
      if (usage_flags & UF_FLAG)
      {
        ...
        if (type_name[0] != '_') {
          pair<tf_iter, bool> redit = builtin_declarators.insert(pair<string,typeflag*>("__" + type_name,nullptr));
          if (redit.second) redit.first->second = new typeflag(*insit.first->second);
          if (type_name[type_name.length() - 1] != '_') {
            redit = builtin_declarators.insert(pair<string,typeflag*>("__" + type_name + "__",nullptr));
            if (redit.second) redit.first->second = new typeflag(*insit.first->second);
          }
        }
        else if (type_name[type_name.length() - 1] != '_') {
          pair<tf_iter, bool> redit = builtin_declarators.insert(pair<string,typeflag*>(type_name + "__",nullptr));
          if (redit.second) redit.first->second = new typeflag(*insit.first->second);
        }
      }
      else flag = 0;
